...
BeTweenApi.interpolation = {
	...
	-- movement jump from y to x and acting as a reverse elastic_out, movement is very slow but increment up in time.
	elastic_in = function (x, y, t)
		t = -(2.0 ^ (10.0 * (t - 1.0)) * math.sin((t - 0.1) * (2.0 * 3.14) / 0.4)) + 1.0
		return BeTweenApi.interpolation.linear(x, y, t)
	end,

	-- movement jump from x to y like an elastic, movement is very fast but slow down in time.
	elastic_out = function (x, y, t)
